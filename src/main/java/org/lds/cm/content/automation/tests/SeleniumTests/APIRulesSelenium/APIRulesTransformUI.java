...
public class APIRulesTransformUI {
    ...
    public void cleanUp() throws SQLException, InterruptedException {
        // Call database and make sure endpoints are actually in state that was changed from the @BeforeClass
        APIRulesStaticMethods.fixAPIRule("ws/v1/transform");
    }
    ...
    @Test (timeOut = 180000)
    public void transformDisabled() throws InterruptedException, SQLException, IOException {
        ...
        ///html/body/div/div/div[2]/div[2]/div[2]/div/table/tbody/tr[30]
        ...
        for (int i = 0; i < apiRows.size(); i++){
            if (apiRows.get(i).getText().contains("ws/v1/transform")) {
                rowNum = i + 1;
                xpathToRow = xpathToRow + "[" + rowNum + "]";
                i = 100;
            }
        }
        System.out.println(xpathToRow);
        ...
        String error = APIRulesStaticMethods.apiFileUploadStatus403(getName.getText(), testFile, Constants.epTransform);
    ...
    @Test (timeOut = 180000)
    public void transformEnabled() throws InterruptedException, SQLException, IOException {
        File testFile = new File(Constants.transformFileStartDir + "/engGeneralConferenceHTML/10/lead-kindly-light.html");
        webDriver = PreTestSetup.setup();
        fono = new FonoSaia();
        ...
        // Click the check box
        String xpathToCheckBox = "//*[@id=\"ruleEnabled\"]";
        String ngEmptyOrNotEmpty = "ng-empty";
        APIRules.clickCheckBox(webDriver, xpathToCheckBox, ngEmptyOrNotEmpty);
        ...
        String error = APIRulesStaticMethods.apiFileUploadStatus(getName.getText(), testFile, Constants.epTransform);
        Assert.isTrue(error.compareTo("")==0, error);
        webDriver.quit();
    }
